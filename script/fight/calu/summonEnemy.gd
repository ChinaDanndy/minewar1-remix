...
func _ready(): 
	...
	for i in groupCount: 
		...
		Global.LevelData[Global.NowLevel]["groups"][i]["times"])
		firstStart(i)
	pass
...
func firstStart(i):
	...
	#print(Global.LevelData[Global.NowLevel]["groups"][i]["firstCD"])	
	await get_tree().create_timer(Global.LevelData[Global.NowLevel]
	["groups"][i]["firstCD"],false).timeout
#		await get_tree().create_timer(
#			((Global.LevelData[Global.NowLevel]["groups"][i-1]["soldierCD"]*
#			Global.LevelData[Global.NowLevel]["groups"][i-1]["soldier"].size())+
#			Global.LevelData[Global.NowLevel]["groups"][i-1]["groupCD"]+
#			Global.LevelData[Global.NowLevel]["groups"][i-1]["groupCDRand"])*
#			Global.LevelData[Global.NowLevel]["groups"][i-1]["times"]-
#			Global.LevelData[Global.NowLevel]["groups"][i-1]["firstCD"]
#			,false).timeout	
...
func summonEnemy(group):
	...
	times[group] += 1
	if Global.LevelData[Global.NowLevel]["groups"][group]["times"] == null: againWait(group)#
	else:
